...
export default async function AdminPanelLayout({
   ...
   * cookie exists. This is where the session is actually validated, so an expired
   * or forged cookie cannot render the panel.
   */
    ...
    <div className="flex min-h-dvh flex-1 flex-col bg-canvas-alt lg:flex-row">
      <aside className="border-b border-canvas-deep bg-canvas lg:w-64 lg:shrink-0 lg:border-r lg:border-b-0">
        <div className="px-6 py-6">
          ...
          <p className="mt-1 text-xs text-ink-faint">{store.storeName}</p>
        </div>
        ...
        <div className="space-y-3 px-6 pb-6">
          <Link
            href="/"
            className="block text-xs font-medium text-ink-faint underline-offset-4 hover:text-ink hover:underline"
          >
            View the site
          </Link>

          <form action={signOut}>
            <p className="truncate text-xs text-ink-faint" title={admin.email}>
              {admin.email}
            </p>
            <button
              type="submit"
              className="mt-1 text-xs font-semibold text-ink-soft underline-offset-4 hover:text-ink hover:underline"
            >
              Sign out
            </button>
          </form>
